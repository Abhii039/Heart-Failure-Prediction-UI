...
if st.session_state.page == 'home':
    # Home Page Content
    st.title("Welcome to the Heart Failure Predictor")
    st.write("""
    ...
    if sex_option == 'Male':
        ...
        creatinine_phosphokinase = st.number_input('Creatinine Phosphokinase (Normal: 38 to 174 mcg/L)', min_value=0)
    else:
        st.write("You selected Female")
        creatinine_phosphokinase = st.number_input('Creatinine Phosphokinase (Normal: 26 to 140 mcg/L)', min_value=0)
    ...
    platelets = st.number_input('Platelets (k/mL)', min_value=0 ,max_value=1000000)
    

    serum_creatinine = st.number_input('Serum Creatinine (Normal: 0.6-1.2 mg/dL)', min_value=0.0, max_value=10.0,format="%.2f")
